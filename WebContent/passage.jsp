	...
	<div class="container">
		<div class="row">
			<div class="col-lg-8 offset-lg-2 col-md-10 offset-md-1">
				...
				<hr />
				<%
					ArrayList<Comment> comments = ViewPostServlet.loadComment(LoginServlet.connection, 200,
								post.getPostID());
				...
				<form id=<%=String.format("\"%s\"", comments.get(i).getPostID())%>
					...
					<div class="comment-preview">
						...
						<input id="comment_id_input" name="comment_id"
							style="display: none"
							value=<%=String.format("\"%s\"", comments.get(i).getCommentID())%>>

						<p class="post-meta">
							<%="作者：" + comments.get(i).getAuther()%>
							<%="评论时间：" + comments.get(i).getPublishTime()%>
						</p>
					</div>
				</form>
				<%
		...
		<div class="row">
			<div class="col-lg-10 offset-lg-2 col-md-10 offset-md-1">
				<form method="post" action="CommentServlet">
					...
					<span><font size="2" color="gray">${error}${message}</font>
					</span> <br>
					<div id="success"></div>
